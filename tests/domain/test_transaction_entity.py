...
def test_treasaction_cration():
    ...
    transaction = TransactionEntity( id=uuid4(),wallet_id=uuid4(), amount=MoneyValueObject(Decimal("100.00"), "MXN"),transaction_type=TransactionType.DEPOSIT, status=TransactionStatus.PENDING, created_at=datetime.now(), updated_at=datetime.now(), )
...
def test_transaction_status_update():
    ...
    transaction = TransactionEntity( id=uuid4(),wallet_id=uuid4(), amount=MoneyValueObject(Decimal("50.00"), "MXN"),transaction_type=TransactionType.WITHDRAWAL, status=TransactionStatus.PENDING, created_at=datetime.now(), updated_at=datetime.now(), )

    completed_transaction = transaction.__class__(**{**transaction.__dict__, "status": TransactionStatus.COMPLETED, "updated_at": datetime.now()})
    
    assert completed_transaction.status == TransactionStatus.COMPLETED
    assert completed_transaction.updated_at is not None
    assert transaction.status == TransactionStatus.PENDING  # Original transaction remains unchanged
